...
from dataset.dataset import VoxDataset
...
global_step = 0
global_epoch = 0
def infonce_loss(x,mel,tao,device):
    x = x.view(-1,5,x.shape[1])
    mel = mel.unsqueeze(1)
...
def train(device, face_encoder,voice_encoder, train_data_loader, test_data_loader, optimizer,
    ...
    for (x, mel) in prog_bar:
        ...
        loss = infonce_loss(x,mel,0.1,device)
        loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    face_encoder = load_checkpoint('/workspace/AVDet/test2023327/bestfaceepoch000000360.pth',face_encoder,optimizer)
    audio_encoder = load_checkpoint('/workspace/AVDet/test2023327/bestaudioepoch000000360.pth',audio_encoder,optimizer)
