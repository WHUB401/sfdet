...
from dataset.dataset import VoxDataset,VoxDatasetv2
...
global_step = 0
global_epoch = 0
def infonce_loss(x,y,z,mel,tao,device):
    p = nn.functional.cosine_similarity(x, mel)
    p = p.unsqueeze(1)
    n1 = nn.functional.cosine_similarity(y, mel)
    n1 = n1.unsqueeze(1)
    n2 = nn.functional.cosine_similarity(z, mel)
    n2 = n2.unsqueeze(1)
    logits = torch.cat([p, n1,n2], dim=1)
    labels = torch.zeros(len(logits), dtype=torch.long, device=device)
    return nn.functional.cross_entropy(logits / tao, labels)
    return loss
logloss = nn.BCELoss()
def cosine_loss(a, v, y):
    d = nn.functional.cosine_similarity(a, v)
    # torch.clamp(d,min=0.0)
...
if __name__ == "__main__":
    ...
    face_encoder = load_checkpoint('/workspace/AVDet/test1/faceepoch000000030.pth',face_encoder,optimizer)
    audio_encoder = load_checkpoint('/workspace/AVDet/test1/audioepoch000000030.pth',audio_encoder,optimizer)
    ...
    for epoch in range(31,300):
        train_loss = train(device,face_encoder,audio_encoder,train_data_loader,test_data_loader,optimizer,checkpoint_dir,epoch)
        ...
        print('-----------------------------------')
        print('start test')
        test_loss = test(device,face_encoder,audio_encoder,test_data_loader,checkpoint_dir,epoch)
