...
def train(model, iterator,epoch, optimizer, criterion, scaler, device):
    epoch_loss = 0
    epoch_acc_1 = 0
    epoch_acc_5 = 0
    ...
    for (x,mel, y) in prog_bar:
        ...
        with autocast(enabled=True):
            ...
            prog_bar.set_description('epoch:{},Train_Loss: {}'.format(epoch,loss))
...
def evaluate(model, iterator, criterion, device):
    ...
    with torch.no_grad():
        ...
        for (x, mel,y) in prog_bar:
            ...
            acc_1, acc_5 = calculate_topk_accuracy(y_pred, y)
            prog_bar.set_description('Eval_Loss: {}'.format(loss))
            epoch_loss += loss.item()
            epoch_acc_1 += acc_1.item()
            epoch_acc_5 += acc_5.item()
